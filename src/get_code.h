...
char *get_code()
{
    ...
    wchar_t *buffer2 = (wchar_t *)buffer;
    bool running = true;
    ...
    while (running)
    {
        ...
        ULONG result = HttpReceiveHttpRequest(handle, requestID, 0, pRequest, requestBufferLength, &bytesRead, 0);
        ...
        if (pRequest->BytesReceived)
        {
            ...
            case HttpVerbGET:
            {
                ...
                ULONG result = HttpSendHttpResponse(handle, pRequest->RequestId, 0, &response, 0, &bytesSend, 0, 0, 0, 0);
