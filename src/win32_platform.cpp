...
#include "mp3-decoder/mp3_decoder_lib.cpp"
...
constexpr uint32_t REDEMPTION_TEXT_LENGTH = 1024;
...
void update_loop(SoundState *soundState, char *mixBuffer)
{
    while (true)
    {
        platform_update_window();
        platform_update_sound(soundState, mixBuffer);

        Sleep(10);
    }
}

void check_redemptions(SoundState *soundState, char *redemptionBuffer, char *encodedRedemptionTextBuffer, HTTPConnection twitchApi,
                       char *token, HTTPConnection streamelementsApi, char *mp3Buffer, short *soundBuffer)
{
    ...
    while (true)
    {
        ...
        {
            const Request getChannelPointRewards = platform_send_http_request(twitchApi,
                                                                              "helix/channel_points/custom_rewards/redemptions?broadcaster_id=80953740&reward_id=713be8e1-266a-4b5c-bf37-c42882ddc845&status=UNFULFILLED", 
                                                                              header, "GET");
        ...
        // Parse the JSON Data, extract UNFULFILLED Requests
        {
            ...
            for (uint32_t requestIdx = 0; requestIdx < requestCount; requestIdx++)
            {
                ...
                // Get MP3 File containing Brian Voice
                {
                    ...
                    const Request getMP3FromStreamelements = platform_send_http_request(streamelementsApi, encodedRedemptionTextBuffer, 0, "GET");
                ...
                if (fileSize)
                {
                    // MP3 Lib Stuff
                    {
                        ...
                        while (fileSize != 0)
                        {
                            ...
                            int sampleCount = mp3dec_decode_frame(&mp3d, (const uint8_t *)mp3File,
                    ...
                    if (s.sampleCount)
                    {
                        ...
                        // Complete the Request using Twtich API
                        {
                            ...
                            const Request completeRedemption = platform_send_http_request(twitchApi, link, header, "PATCH",
                                                                                          "{\"status\": \"FULFILLED\"}");
...
int main()
{
    ...
    // Load MP3 File from Disk to play it
    {
        // uint32_t fileSize = 0;
        // char *mp3File = platform_read_file("mp3data.mp3", &fileSize);
        // parse_mp3(mp3File, fileSize, pcmBuffer);
    ...
    const HTTPConnection streamelementsApi = platform_connect_to_server("api.streamelements.com");
    ...
    // Get Code, used in OAuth2
    {
        ...
        // const Request getRefreshToken = platform_send_http_request(twitchID, oAuthReq, 0);
    ...
    // Get Broadcaster ID
    {
        ...
        const Request getBroadcasterID = platform_send_http_request(twitchApi, "helix/users?login=cakez77", buffer, "GET");
    ...
    // Create Channel Point Redemption
    {
        ...
        // const Request createChannelPointRewards = platform_send_http_request(twitchApi,
    ...
    std::thread checkRedemptions(check_redemptions, globalSoundState, redemptionBuffer, buffer, twitchApi, token, streamelementsApi,
...
const Request platform_send_http_request(const HTTPConnection connection, char *url, char *header, char *method, char *data)
{
    ...
    // Conversion bullshit here
    wchar_t wUrl[2048] = {};
    int length = platform_convert_to_wchar(url, wUrl, 2048);
...
bool platform_receive_http_data(const Request request, char *outBuffer, uint32_t bufferSize, uint32_t *outRecievedBytes)
{
    if (!(request.httpHandle))
    {
        CAKEZ_ASSERT(0, "Invalid Request: URL: %s, Method: %s, Header: %s",
