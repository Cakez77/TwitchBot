...
void add_rude_person(RudePersonCollection *rudePersonCollection, const char *name, uint32_t nameLength)
{
                ...
                while (true)
                {
                    ...
                    if (m.length > 0)
                    {
                        ...
                        if (strstr(msgBegin, "!commands"))
                        {
                            ...
                            const char *msgPrefix = "PRIVMSG #cakez77 : Possible commands are: !rude @user, ";
                            uint32_t offset = 0;
                            ...
                            if (nameLength)
                            {
                                ...
                                for (uint32_t i = 0; i < rudePersons.rudePersonCount; i++)
                                {
                                    ...
                                    if (str_cmp(rp->name, rudePersonName))
                                    {
                                        ...
                                        platform_write_file(RUDE_PERSON_FILE_NAME, (const char *)&rudePersons, sizeof(RudePersonCollection), true);
                                        break;
                                    }
                                }
