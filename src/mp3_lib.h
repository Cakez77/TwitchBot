...
{
    // 0...7
    uint32_t bitIdx = bs->pos & 7;
    uint32_t next, cache = 0;
    int shl = n + bitIdx;
...
{
    unsigned int sampleCount = hdr_frame_samples(h);
    unsigned int kbpsBitrate = hdr_bitrate_kbps(h);
    unsigned int sampleRateHz = hdr_sample_rate_hz(h);
